---
name: eds-website-builder-ema
description: Build a complete AEM Edge Delivery Services website from inside Experience Catalyst (aemcoder.adobe.io / EMA). Variant of eds-website-builder adapted for the ExCat sandbox — writes local content to `content/` instead of `drafts/`, and skips direct DA upload / preview / browser-open steps because Experience Catalyst handles those via its UI. Use when working inside Experience Catalyst and the user asks to create an EDS site, build pages from a briefing, or generate blocks and drafts.
---

# EDS Website Builder (Experience Catalyst variant)

Build a complete AEM Edge Delivery Services website from inside **Experience Catalyst** (aemcoder.adobe.io / "EMA"). This skill is a sibling of `eds-website-builder` with two behavioural differences tuned for the ExCat sandbox:

1. **Local content lives under `content/{sitename}/`**, not `drafts/{sitename}/` — Experience Catalyst's built-in AEM up is configured to serve the `content/` folder.
2. **No direct DA upload or preview invocation from the skill.** The user exports to DA and triggers previews through the Experience Catalyst UI. The skill's job ends when the files exist in `content/` and structural validation passes.

## Operating in Experience Catalyst

Assume the following about the ExCat environment:

- **Project already exists** — do not clone `aem-boilerplate` or run `gh repo create`. The working directory is the project.
- **AEM up is already running** against `content/` — do not invoke `npx @adobe/aem-cli up` yourself.
- **No DA credentials inside the LLM sandbox** — don't attempt `./tools/upload-to-da.sh` or `./tools/preview-all.sh`. Mention that the user should click "Export to DA" / preview in the ExCat UI.
- **No `open`, no `gh`, no `wrangler` in the sandbox.** Any step that needs them must be flagged as "run this in your local terminal, outside Experience Catalyst".
...
- User has a briefing and wants pages generated into `content/{sitename}/`
- User asks about EDS architecture, block development, or local content authoring in the ExCat context
...
```bash
grep -rn 'src="/\|src="\.\.' content/ --include='*.html' && echo "BLOCKED: fix local paths before uploading" || echo "OK: no local paths"
```
...
**In Experience Catalyst, skip Phases 1.1, 1.2, and 1.4** — the project is already cloned, already connected to GitHub, and AEM up is already running against `content/`. The remainder of Phase 1 (content source, project structure) is background reading only.

### 1.1 Clone the Boilerplate (skip in ExCat)
...
In ExCat the LLM sandbox has no `wrangler` or Cloudflare credentials. Instead:

1. Check if an existing site CDN already hosts a suitable image (see image-generation reference for the catalogue) and reuse that URL directly.
2. If a new CDN deployment is required, tell the user to run these commands in their local terminal (outside ExCat):
...
### 6.4 Update HTML References

Replace placeholder URLs in content HTML with the public CDN URLs. In ExCat, image deployment and DA export are handled via the workspace UI — no CLI upload step.

```bash
find content -name '*.plain.html' -exec sed -i '' 's|/images/|https://my-site-images.pages.dev/|g' {} \;
```
...
- **Local drafts** — Create `.plain.html` files in `content/` for development without a CMS
...
4. Create nav + footer   → Write content/{sitename}/nav.plain.html & footer.plain.html
...
- The **output path** (e.g. `content/{sitename}/efficacy.plain.html`)
...
for (const page of pages) {
  Agent({
    ...
    prompt: `
      Create the file: content/${sitename}/${page.filename}
...
FAILED=0
for url in $(grep -oh 'https://[^"]*\.jpeg' content/{sitename}/*.plain.html | sort -u | head -3); do
...
```bash
grep -rn 'src="/' content/{sitename}/ --include='*.html' && echo "BLOCKED: fix local paths before uploading" || echo "OK: no local paths"
```

**Step 1b — Verify no placeholder links in nav/footer** (blocks upload if any are found):
```bash
grep -Pn 'href="/{sitename}/"' content/{sitename}/nav.plain.html content/{sitename}/footer.plain.html | grep -v 'img src' | grep -v 'Login' && echo "BLOCKED: placeholder links found — replace with briefing URLs" || echo "OK: no placeholder links"
```
...
grep -c 'class="accordion"' content/{sitename}/*.plain.html
...
## Phase 8: DA Export (Handled by ExCat)

In Experience Catalyst, DA export is handled outside the LLM sandbox — the workspace service has the credentials and exposes an "Export to DA" action in the ExCat UI. Do not attempt to call DA APIs or run `./tools/upload-to-da.sh` from here.

Before handing off, verify no local asset paths remain (DA converts these to `about:error`):
```bash
grep -rn 'src="/\|src="\.\.' content/ --include='*.html' && echo "BLOCKED: fix local paths" || echo "OK"
```

Then tell the user to export via the ExCat UI.
