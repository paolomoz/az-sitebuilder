...
> **Not applicable inside Experience Catalyst.** ExCat's workspace service owns DA credentials and exposes an "Export to DA" action in the UI. The LLM sandbox has no DA keys and must not call the scripts below. This file remains only as background about how DA integration works outside ExCat.
...
# If no file specified, uploads all .plain.html files in content/
...
#    (files in content/*.plain.html)

# 2. Test locally with dev server
aem up --no-open --html-folder content
...
```bash
./tools/upload-to-da.sh content/products/aspirin.plain.html
```
