...
Draft files in the `content/` folder follow `.plain.html` structure:
...
Create `content/nav.plain.html` for the header:
...
1. Open `content/nav.plain.html` (or the CMS nav page)
...
1. Open `content/nav.plain.html` and `content/footer.plain.html`
...
When running with `--html-folder content`, the AEM CLI serves pages at `/content/{sitename}/` not `/{sitename}/`. The `getSiteRoot()` function returns the first URL path segment, so it returns `/drafts` instead of `/{sitename}`.

This means nav loads from `/content/nav` and footer from `/content/footer`. For local dev to work, you must also place copies of `nav.plain.html` and `footer.plain.html` at the `content/` root level:
