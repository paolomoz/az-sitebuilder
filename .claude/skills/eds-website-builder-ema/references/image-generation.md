...
```bash
grep -rn "unsplash.com\|placehold.co" content/ --include="*.html" || echo "All clean"
```

### 4. Upload to DA

In ExCat, DA export and preview refresh are performed from the workspace UI — no CLI scripts are available inside the LLM sandbox. After export, images are served by the AEM media CDN.
